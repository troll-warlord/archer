"""
models/aws/elb.py — Elastic Load Balancing models.

Covers: Application Load Balancer (ALB), Network Load Balancer (NLB),
...
class RedirectConfig(BaseModel):
    """Inline redirect action for a listener (e.g. HTTP → HTTPS)."""

    model_config = ConfigDict(frozen=True)

    port: str = "443"
    protocol: Literal["HTTP", "HTTPS"] = "HTTPS"
    status_code: Literal["HTTP_301", "HTTP_302"] = "HTTP_301"
...
class ListenerConfig(BaseModel):
    ...
    # Forward to a named target group declared in the same ALB/NLB block.
    target_group_ref: str | None = None
    # HTTPS / TLS only
    certificate_arn: str | None = None
    ssl_policy: str | None = None
    # Redirect action — mutually exclusive with target_group_ref.
    redirect: RedirectConfig | None = None
    # Legacy boolean shorthand kept for backward-compat; prefer redirect: above.
